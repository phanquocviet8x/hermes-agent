...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Register handlers
            self._app.add_handler(CommandHandler("menu", self._handle_menu_command))
            self._app.add_handler(CommandHandler("modelmenu", self._send_main_menu))
            self._app.add_handler(CommandHandler("provider", self._handle_provider_command))
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            # Arrange buttons in the same 2-column layout as the previous release.
    ...
    async def _handle_callback_query(
        ...
        # --- Sula model controls ---
        # These callbacks are emitted by the profile-integrated inline menu.
        # Reuse the same text-command paths as the persistent Telegram controls.
        if data in {"switch_model", "model"} and query_message is not None:
            if data == "switch_model":
                await self._send_router_picker(query_message)
            else:
                event = self._build_message_event(query_message, MessageType.COMMAND)
                event.text = "/model"
        ...
        # --- OmniRoute upstream-provider picker callbacks ---
        if data.startswith(("opv:", "opm:", "opg:", "opa", "opb", "opx", "opn")):
            if not self._is_callback_user_authorized(
                str(getattr(query.from_user, "id", "")),
                ...
                await query.answer(text="⛔ Bạn không có quyền đổi provider.")
                return
            chat_id = str(query_chat_id) if query_chat_id is not None else ""
            if chat_id:
                await self._handle_omniroute_provider_callback(query, data, chat_id)
            return
        ...
        # --- Profile-local integrated menu callbacks ---
        integrated_chats = getattr(self, "_integrated_menu_active_chats", set())
        if query_chat_id is not None and str(query_chat_id) in integrated_chats:
            if await self._forward_to_integrated_menu(update):
                return
    ...
    def _router_config_dir(self) -> _Path:
        """Return this adapter/profile's router directory, never another profile's."""
        configured = str(self.config.extra.get("router_config_dir") or "").strip()
        if configured:
    ...
    def _router_config_path(self, key: str) -> _Path:
        _label, filename = self._ROUTER_CONFIGS[key]
        return self._router_config_dir() / filename

    def _read_current_router_status(self) -> tuple[str, str, str]:
        """Return (router_key, router_label, model) without exposing secrets."""
        config_path = self._router_config_dir() / "config.yaml"
        current_provider = ""
        ...
        if "freeclaude" in provider_l or "freeclaude" in model_l or "fcc" in provider_l:
            return "freeclaudecode", "Free Claude Code", current_model
    ...
    async def _handle_router_callback(self, query, data: str) -> None:
        ...
        if not self._is_callback_user_authorized(
            ...
            return
        label, source_name = self._ROUTER_CONFIGS[key]
        source = self._router_config_path(key)
        dest = self._router_config_dir() / "config.yaml"
        try:
            ...
            await query.edit_message_text(
                text=self.format_message(
                    ...
                    f"File gốc vẫn giữ nguyên: `{source}`\n"
                    f"Đã copy ra: `{dest}`\n"
                    f"Model hiện tại: `{current_model}`"
                ),
                parse_mode=ParseMode.MARKDOWN_V2,
    ...
    def _fetch_omniroute_provider_catalog(self, router_key: str = "omniroute") -> list[dict]:
        ...
        for row in rows:
            ...
            generic_owners = {"combo", "apikey", "apimode", "unknown", "provider"}
            provider = (
                owned_by
                if safe_provider.fullmatch(owned_by) and owned_by not in generic_owners
                else raw_provider
            )
            if not safe_provider.fullmatch(provider):
                continue
            model_suffix = model_id.split("/", 1)[1]
    ...
    def _build_omniroute_provider_keyboard(self, providers: list[dict]) -> InlineKeyboardMarkup:
        buttons = [
            InlineKeyboardButton(
                f"{entry['provider']} ({len(entry['models'])})", callback_data=f"opv:{index}"
            )
            for index, entry in enumerate(providers)
        ]
        rows = [buttons[i : i + 2] for i in range(0, len(buttons), 2)]
        rows.append([InlineKeyboardButton("✗ Cancel", callback_data="opx")])
        return InlineKeyboardMarkup(rows)

    def _build_omniroute_model_keyboard(self, state: dict, page: int) -> tuple[InlineKeyboardMarkup, str]:
        models = state.get("models", [])
        total = len(models)
        page_size = self._OMNIROUTE_PROVIDER_PAGE_SIZE
        ...
        for index in range(start, end):
            ...
            if len(label) > 38:
                label = label[:35] + "..."
            buttons.append(InlineKeyboardButton(label, callback_data=f"opm:{index}"))
        rows.extend(buttons[i : i + 2] for i in range(0, len(buttons), 2))
        if pages > 1:
            nav = []
            if page > 0:
                nav.append(InlineKeyboardButton("◀ Prev", callback_data=f"opg:{page - 1}"))
            nav.append(InlineKeyboardButton(f"{page + 1}/{pages}", callback_data="opn"))
            if page + 1 < pages:
                nav.append(InlineKeyboardButton("Next ▶", callback_data=f"opg:{page + 1}"))
            rows.append(nav)
        rows.append([
            InlineKeyboardButton("◀ Providers", callback_data="opb"),
            InlineKeyboardButton("✗ Cancel", callback_data="opx"),
        ])
        info = f" ({start + 1}–{end} of {total})" if total else ""
    ...
    async def _handle_provider_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        msg = self._effective_update_message(update)
        if not msg or not self._is_user_authorized_from_message(msg):
            return
        # Telegram clients keep the previous persistent ReplyKeyboard until the
        # bot sends a new one. Refresh it here so an already-open DM immediately
        # receives the Provider button after a gateway upgrade/restart.
    ...
    async def _handle_omniroute_provider_callback(self, query, data: str, chat_id: str) -> None:
        state = getattr(self, "_omniroute_provider_state", {}).get(chat_id)
        if data == "opn":
            await query.answer()
            return
        if data == "opx":
        ...
        if data == "opb":
            ...
            await query.answer()
            return
        if data.startswith("opv:"):
            try:
                entry = state["providers"][int(data[4:])]
            except (ValueError, IndexError, KeyError):
                await query.answer(text="Provider không hợp lệ.")
                return
            state["selected_provider"] = entry["provider"]
            state["models"] = entry["models"]
            keyboard, info = self._build_omniroute_model_keyboard(state, 0)
            await query.edit_message_text(
                f"🔌 Provider: {entry['provider']}{info}\n\nChọn Auto hoặc model cố định:",
                reply_markup=keyboard,
            )
            await query.answer()
            return
        if data.startswith("opg:"):
            try:
                page = int(data[4:])
            except ValueError:
                await query.answer(text="Trang không hợp lệ.")
                return
            keyboard, info = self._build_omniroute_model_keyboard(state, page)
            await query.edit_message_text(
                f"🔌 Provider: {state.get('selected_provider', '')}{info}\n\nChọn Auto hoặc model cố định:",
                reply_markup=keyboard,
            )
            await query.answer()
            return
        ...
        if data.startswith("opm:"):
            try:
                model_id = state["models"][int(data[4:])]
            except (ValueError, IndexError, KeyError):
    ...
    async def _handle_text_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if text_clean in {"Switch mode", "Switch Mode", "Switch model", "Switch Model", "Free Claude Code"}:
            await self._send_router_picker(msg)
            return
        if text_clean in {"Provider", "provider"}:
            await self._send_omniroute_provider_picker(msg)
            return
        ...
        if text_clean in {"Menu", "menu"}:
            if await self._send_integrated_menu(msg):
                return
            await self._send_main_menu(update, context)
            return
        integrated_chats = getattr(self, "_integrated_menu_active_chats", set())
        if chat_id_str in integrated_chats and self._integrated_menu_has_active_conversation():
            if await self._forward_to_integrated_menu(update):
                return
    ...
    async def _handle_menu_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Open the profile's integrated menu when configured; otherwise show Hermes menu."""
        msg = self._effective_update_message(update)
        if not msg or not self._is_user_authorized_from_message(msg):
            return
        if await self._send_integrated_menu(msg):
    ...
    def _load_integrated_menu_module(self):
        """Load a profile-local menu module without starting its polling loop."""
        module_path = str(self.config.extra.get("integrated_menu_module") or "").strip()
        if not module_path:
            return None
        path = _Path(module_path).expanduser().resolve()
        try:
            allowed_root = self._router_config_dir().resolve()
            path.relative_to(allowed_root)
        except (OSError, ValueError):
            logger.error("[%s] Refusing integrated menu module outside profile home", self.name)
            return None
        if not path.is_file():
            logger.error("[%s] Integrated menu module not found: %s", self.name, path)
            return None
        cached = getattr(self, "_integrated_menu_module", None)
        if cached is not None and getattr(self, "_integrated_menu_module_path", None) == str(path):
            return cached
        try:
            import importlib.util
            name = f"hermes_profile_integrated_menu_{abs(hash(str(path)))}"
            spec = importlib.util.spec_from_file_location(name, path)
            if spec is None or spec.loader is None:
                return None
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            self._integrated_menu_module = module
            self._integrated_menu_module_path = str(path)
            return module
        except Exception:
            logger.exception("[%s] Failed loading integrated menu module", self.name)
            return None

    async def _send_integrated_menu(self, msg) -> bool:
        """Render the copied Sula menu through this adapter's single Telegram poller."""
        module = self._load_integrated_menu_module()
        builder = getattr(module, "get_main_menu", None) if module is not None else None
        if not callable(builder):
            return False
        try:
            user_id = int(getattr(getattr(msg, "from_user", None), "id", 0) or 0)
            active_chats = getattr(self, "_integrated_menu_active_chats", None)
            if active_chats is None:
                active_chats = set()
                self._integrated_menu_active_chats = active_chats
            active_chats.add(str(getattr(getattr(msg, "chat", None), "id", "")))
            await msg.reply_text(
                "🛍️ **MENU SULA**\n\n✨ Chọn chức năng bên dưới:",
                reply_markup=builder(user_id),
                parse_mode="Markdown",
            )
            return True
        except Exception:
            logger.exception("[%s] Failed rendering integrated menu", self.name)
            await msg.reply_text("❌ Menu Sula tạm thời chưa mở được. Vui lòng thử lại sau.")
            return True

    async def _integrated_menu_application(self):
        """Return the embedded menu Application initialized against this adapter's Bot."""
        app = getattr(self, "_integrated_menu_app", None)
        if app is not None:
            return app
        module = self._load_integrated_menu_module()
        factory = getattr(module, "build_application", None) if module is not None else None
        if not callable(factory):
            return None
        try:
            app = factory(self.config.token)
            await app.initialize()
            # The embedded app only dispatches handlers and never starts polling.
            # Its Bot uses the same credential while the incoming Update remains
            # bound to Hermes' connected Bot for callback replies.
            self._integrated_menu_app = app
            return app
        except Exception:
            logger.exception("[%s] Failed initializing integrated menu handlers", self.name)
            return None

    async def _forward_to_integrated_menu(self, update) -> bool:
        """Forward an integrated-menu callback to the copied module's handlers."""
        app = await self._integrated_menu_application()
        if app is None:
            return False
        try:
            ...
            logger.exception("[%s] Integrated menu callback failed", self.name)
            query = getattr(update, "callback_query", None)
            if query is not None:
                try:
                    await query.answer(text="Menu Sula đang lỗi, vui lòng thử lại.")
                except Exception:
                    pass
            return True

    def _integrated_menu_has_active_conversation(self) -> bool:
        """Whether the embedded menu currently expects follow-up text."""
        app = getattr(self, "_integrated_menu_app", None)
        if app is None:
            return False
        try:
            ...
            logger.debug("[%s] Could not inspect integrated conversation state", self.name, exc_info=True)
        return False
...
def _apply_yaml_config(yaml_cfg: dict, telegram_cfg: dict) -> dict | None:
    ...
    for _key in (
        ...
        "free_response_topics", "router_config_dir", "integrated_menu_module",
    ):
